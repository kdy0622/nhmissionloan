...
본 메일은 서울축산농협 상계역지점 특판 시스템을 통해 자동 생성되었습니다.
  ...
  if (currentStep === Step.START) {
    return (
      <div className="min-h-screen bg-white">
        <section className="relative py-20 flex flex-col items-center justify-center overflow-hidden bg-[#228B22] text-white">
          ...
          <div className="relative z-10 text-center px-4 max-w-5xl w-full">
            ...
            <div className="bg-white rounded-3xl p-10 mb-14 shadow-2xl text-gray-800 border-4 border-yellow-400">
              <h3 className="text-3xl font-black border-b-4 border-[#228B22] pb-4 mb-8 text-[#228B22] text-center">🚀 특별 우대 혜택</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-8">
                ...
                <div className="flex flex-col items-center p-6 bg-green-50 rounded-2xl border-2 border-green-100">
                  ...
                  <span className="text-5xl font-black text-[#228B22]">500억 원</span>
                </div>
              </div>
        ...
        <section className="py-24 px-6 bg-white text-center">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-4xl font-black mb-10 text-[#228B22] italic">대한민국 최대 규모, 서울축산농협</h2>
            <div className="space-y-6 text-2xl text-gray-700 leading-relaxed font-medium">
              <p>서울축산농협은 서울지역 전체에 <span className="font-bold text-[#228B22] underline">28개의 금융점포</span>를 보유한 지역농협으로서 대한민국 최대 규모를 자랑합니다.</p>
        ...
        <section className="py-20 px-4 bg-gray-50">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-4xl font-black text-center mb-16 text-[#228B22] drop-shadow-sm">📞 대환 대출 전담 상담 창구 (상계역지점)</h2>
            <div className="grid md:grid-cols-3 gap-8">
              ...
              <a href="tel:010-7399-5380" className="bg-white p-10 rounded-3xl shadow-lg border-t-8 border-[#228B22] text-center transition-all hover:bg-green-50 active:scale-95 group">
                <span className="text-gray-500 text-xl font-bold">지점장</span>
                <h4 className="text-3xl font-black mb-4">김대윤</h4>
                <div className="text-[#228B22] font-black text-2xl group-hover:underline">010-7399-5380</div>
                <p className="text-sm text-gray-400 mt-2 font-bold italic">번호클릭시 자동연결</p>
              </a>
            </div>
            
            <div className="mt-16 bg-white p-12 rounded-3xl shadow-md border-2 border-gray-200 text-center">
              <h3 className="text-3xl font-black mb-6 text-[#228B22]">📠 온라인 및 팩스 상담 신청 안내</h3>
              <p className="text-2xl font-bold text-gray-700 mb-4">사무실 팩스 번호: <a href="tel:02-3392-3916" className="underline text-[#228B22] font-black">02-3392-3916</a></p>
  ...
  return (
    <div className="min-h-screen pb-20 bg-white">
      <header className="relative bg-[#228B22] py-12 px-4 text-center overflow-hidden shadow-lg">
        ...
        <div className="relative z-10">
          ...
          <p className="text-white text-2xl opacity-95 font-bold">서울축산농협 상계역지점 대환 대출 전용</p>
        </div>
      </header>

      <main className="max-w-4xl mx-auto mt-8 p-8 bg-white shadow-2xl rounded-3xl border border-gray-100">
        <div className="flex justify-between mb-12 border-b-2 border-gray-100 pb-8 overflow-x-auto">
          {[1, 2, 3, 4, 5].map((s) => (
            <div key={s} className="flex flex-col items-center min-w-[60px] flex-1">
              <div className={`w-14 h-14 rounded-full flex items-center justify-center text-2xl font-black mb-2 transition-colors ${currentStep >= s ? 'bg-[#228B22] text-white shadow-lg' : 'bg-gray-200 text-gray-500'}`}>
                {s}
              </div>
              <span className={`text-xl font-bold ${currentStep >= s ? 'text-[#228B22]' : 'text-gray-400'}`}>Step {s}</span>
            </div>
          ))}
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          {currentStep === Step.CHURCH_INFO && (
            <div className="space-y-6">
              <h2 className="text-3xl font-black border-b-4 border-[#228B22] pb-3 mb-8">Step 1: 교회 기본 정보</h2>
          ...
          {currentStep === Step.LOAN_STATUS && (
            <div className="space-y-6">
              <h2 className="text-3xl font-black border-b-4 border-[#228B22] pb-3 mb-8">Step 2: 대출 및 재정 현황</h2>
          ...
          {currentStep === Step.COLLATERAL && (
            <div className="space-y-6">
              <h2 className="text-3xl font-black border-b-4 border-[#228B22] pb-3 mb-8">Step 3: 담보물 소재지 (지번 목록)</h2>
              <div className="space-y-5">
                {formData.collaterals.map((collateral, index) => (
                  <div key={collateral.id} className="flex gap-4 items-center">
                    <span className="text-2xl font-black text-[#228B22] w-24 text-center">{index + 1}번</span>
                    <input 
                      type="text" 
                      className="flex-1 p-6 border-2 border-gray-200 rounded-2xl text-2xl font-bold focus:border-[#228B22] focus:outline-none transition-all shadow-sm"
              ...
              <button 
                ...
                className="w-full mt-8 py-8 border-4 border-dashed border-[#228B22] rounded-3xl text-[#228B22] font-black text-3xl hover:bg-green-50 active:scale-95 transition-all shadow-md"
              >
                + 담보물 추가 입력창 생성
              </button>
            </div>
          ...
          {currentStep === Step.ATTACHMENT && (
            <div className="space-y-10">
              <h2 className="text-3xl font-black border-b-4 border-[#228B22] pb-3 mb-8">Step 4: 교회 담당자 정보</h2>
          ...
          {currentStep === Step.SUBMISSION && (
            <div className="space-y-10">
              <h2 className="text-3xl font-black border-b-4 border-[#228B22] pb-3 mb-8">Step 5: 약관 동의 및 최종 신청</h2>
              <div className="p-10 bg-[#FAF9F6] border-4 border-gray-100 rounded-3xl text-2xl leading-relaxed text-gray-700 shadow-inner font-medium">
              ...
              <div className="space-y-10 bg-green-50 p-12 rounded-3xl border-4 border-[#228B22] shadow-2xl">
                <div className="flex items-center gap-6">
                  <input type="checkbox" id="agreement" className="w-12 h-12 accent-[#228B22] cursor-pointer" checked={formData.agreement} onChange={(e) => updateFormData({ agreement: e.target.checked })} />
          ...
          <div className="flex gap-6 pt-12 sticky bottom-0 bg-white/95 backdrop-blur-md py-6 shadow-[0_-15px_40px_rgba(0,0,0,0.1)]">
            ...
            <button type="submit" className="flex-[2] py-7 bg-[#228B22] text-white rounded-3xl text-3xl font-black border-b-8 border-green-800 hover:bg-green-700 active:scale-95 transition-all">
      ...
      <footer className="mt-20 text-center text-gray-500 pb-20 px-6">
        ...
        <p className="mt-6 text-xl font-medium">상담: <a href="tel:02-2181-5228" className="text-[#228B22] underline font-black">02-2181-5228</a></p>
        <p className="text-lg mt-2 text-gray-400 font-bold italic">사무실 팩스: <a href="tel:02-3392-3916" className="underline text-[#228B22]">02-3392-3916</a></p>
      </footer>
    </div>
  );
};
...
const InputGroup: React.FC<{label: string, value: string, onChange: (v: string) => void, type?: string, placeholder?: string}> = ({label, value, onChange, type="text", placeholder}) => (
  <div className="flex flex-col gap-4">
    ...
    <input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="w-full p-8 border-3 border-gray-200 rounded-3xl text-3xl font-bold focus:border-[#228B22] focus:outline-none shadow-sm focus:ring-4 focus:ring-green-100 transition-all" placeholder={placeholder} />
  </div>
);
